...
const totalLikes = (blogs) => {
  const total = blogs.reduce((acc, current) => acc + current.likes, 0)
  return total
}
...
const favoriteBlog = (blogs) => {
  ...
  const mostLiked = blogs.reduce((best, current) =>
...
const mostBlogs = (blogs) => {
  ...
  const mostProfilic = Object.entries(blogCounts).reduce((max, current) =>
...
const mostLikes = (blogs) => {
  ...
  const mostLiked = Object.entries(likeCounts).reduce((max, current) =>
    current[1] > max[1] ? current : max,
  )
